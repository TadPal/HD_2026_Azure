...
from .core import (
    EntraIDClient,
    make_easyauth_headers_from_claims,
...
class EntraEasyAuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        external_base_url: (
            str | None
        ) = None,  # veřejná URL za reverse proxy (např. https://app.example.com)
    ...
    @staticmethod
    def _get_header(
        scope_headers: list[tuple[bytes, bytes]], key_lower: bytes
    ) -> Optional[bytes]:
    ...
    @staticmethod
    def _set_header(
        scope_headers: list[tuple[bytes, bytes]], key: str, value: str
    ) -> None:
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not auth_hdr and self.propagate_cookie_to_bearer and token_from_cookie:
            bearer_val = (
                ...
                else f"Bearer {token_from_cookie}"
            )
        ...
        try:
            ...
            claims = await self.entra.validate_bearer_token(
                (
                    auth_hdr.decode()
                    if isinstance(auth_hdr, (bytes, bytearray))
                    else auth_hdr
                )
                or ""
            )
            ...
            logging.info(("token is NOT ok" f" (error: {e}, "))
...
def require(
    scopes: Optional[Iterable[str]] = None, roles: Optional[Iterable[str]] = None
):
