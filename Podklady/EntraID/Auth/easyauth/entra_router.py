...
def create_entra_router(
    *,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(token_url, data=token_params) as entra_response:
                if entra_response.status != 200:
                    ...
                    logging.error(
                        f"Error fetching token: {entra_response.status} - {text}"
                    )
                    return RedirectResponse(url="/")
    ...
    async def homepage(request: Request):
        ...
        message = (
            "<div>Hello, Guest! Please <a href='/login?next=/me'>log in</a></div>."
        )
        if token_data:
